...
Worker.prototype.start = function(){
  ...
  if (this.server && this.server.listenFD) {
    this.server.on('listening', function(){
      var group = self.options ? self.options.group : undefined;
      if (group) process.setgid(group);
      var user = self.options ? self.options.user : undefined;
...
Worker.prototype.close = function(){
  ...
  if (server) {
	...
	var timeoutStart = Date.now();
		//TODO remove legion dependency.
	  require('legion').socket.closeServices();

    // check pending connections
    setInterval(function(){
      var timeleft = self.timeout ? (self.timeout - (Date.now() - timeoutStart) ) : 'infinite';
	  if(process.listeners('can shutdown').length > 0) {
          process.emit('can shutdown', timeleft, function( err, clients ) {
            if(err || (clients === 0)) return self.destroy();
		    self.master.call('workerWaiting', clients);
          })
      } else {
        self.destroy();
      }
...
Worker.prototype.onBroadcast = function(id, msg) {
	process.emit('clusterBroadcast', id, msg);
};
